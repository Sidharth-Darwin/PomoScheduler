...
from pomo.storage import init_db, create_daily_task, get_pending_tasks, delete_task
...
def send_to_daemon(payload: dict, retries: int = 1) -> dict:
    """Send JSON to daemon. If daemon is dead, silently spawn it and try again."""
    ...
    try:
        ...
        if retries > 0:
            ...
            time.sleep(0.5)  # Give it half a second to boot
...
@app.command()
def status(
    as_json: bool = typer.Option(False, "--json", help="Output JSON for AI agents"),
    short: bool = typer.Option(
        False, "--short", help="Short output for Polybar/Waybar"
    ),
):
    """Get the current background timer status."""
    ...
    if short:
        ...
        if phase == "WORK":
            ...
            print(
                f"BREAK {mins:02d}:{secs:02d} ({task.get('pomodoro_current', 1) - 1}/{task.get('pomodoro_total', 0)})"
            )
        elif phase == "PAUSED":
...
def daemon():
    init_db()
    typer.secho(
        "Starting Pomo daemon... (Press Ctrl+C to stop)", fg=typer.colors.MAGENTA
    )
...
@app.command()
def start(task_id: int = typer.Argument(...)):
    response = send_to_daemon({"action": "start", "task_id": task_id})
    if response.get("status") == "success":
        typer.secho(response.get("message"), fg=typer.colors.GREEN)
    else:
        typer.secho(f"Error: {response.get('message')}", fg=typer.colors.RED)
